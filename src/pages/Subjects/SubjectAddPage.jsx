...
export default function SubjectAddPage() {
  ...
  // ค่าเริ่มต้นว่าง ๆ สำหรับการเพิ่มใหม่
  ...
  const handleBack = () => navigate("/subjectslist");
  const handleCancel = () => navigate("/subjectslist");

  const handleSave = async (e) => {
    e.preventDefault();

    if (!form.id?.trim() || !form.name?.trim()) {
      window.alert("กรุณากรอก Subject ID และ Subject Name");
      return;
    }

    // TODO: เรียก API POST เพื่อสร้างวิชาใหม่

    console.log("CREATE SUBJECT =>", form);
